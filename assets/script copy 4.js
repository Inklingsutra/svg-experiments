...
hexData.forEach((h, i) => {
  ...
  // sharpen falloff (important)
  //influence = Math.pow(influence, 2);

  // Add motion modulation
  influence *= 0.5 + 0.5 * Math.sin(elapsed * 4 + i * 0.3);

  // Beat sync
  //influence *= 0.6 + 0.4 * Math.sin(elapsed * 6);
  const wave = Math.sin(dist * 0.05 - elapsed * 4);
  influence *= 0.5 + 0.5 * wave;

  // Apply visuals
  const opacity = h.baseOpacity + influence * 0.6;
